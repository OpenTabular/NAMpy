...
# --- DATA ---
def get_independent_synthetic_data(n_samples: int = 1000):
    """
...
if __name__ == "__main__":
    ...
    try:
        ...
        results = {}
        for experiment_idx, n_samples in enumerate([1000, 2000, 4000, 8000, 16000]):
            data, true_effects = get_independent_synthetic_data(n_samples=n_samples)
    ...
    nll, crps = {}, {}
    for experiment_id, results_dict in results.items():
        final_params, submodel_contributions = model.predict(
            data_loader=results_dict["data_loader"]
        )
        ...
        nll[experiment_id] = -jnp.mean(jnp.sum(log_probs, axis=-1))
        ...
        crps[experiment_id] = score.mean(axis=0)

    for i, (metric, name) in enumerate(
            zip([nll, crps], ["NLL", "CRPS"]),
    ):
        ax_to_plot = ax[i]
        sns.lineplot(
            x=[
                experiment_id.split("_")[-1].replace("N", "")
                for experiment_id in metric.keys()
            ],
            y=[float(val) for val in metric.values()],
            ax=ax_to_plot,
            label=name,
            color="black"
        )
        ax_to_plot.set_xlabel("Experiment ID", fontsize=12)
        ax_to_plot.set_ylabel(name, fontsize=12)
        ax_to_plot.set_title(name, fontsize=14)
        ax_to_plot.legend(loc='upper center', fontsize=12, frameon=False)
        ax_to_plot.grid(True)
