...
| 7. Prediction, inference, and diagnostics | Term filters plus unconditional SE; unconditional term inference for random effects, tensors, linked terms, FS, SZ, factor-by, and `ti`; shared-component link/response/SE/lpmatrix prediction and per-target decomposition; structured newdata; explicit unseen-level and NA rejection; and diagnostics/snapshot round trips. Gaussian-inverse REML directly covers post-fit likelihood criteria, covariance modes, residuals, summary/ANOVA, and k-check. Persistence covers shared-component layouts, random effects, `te`, `ti`, linked terms, FS, SZ, factor-by, `gaulss`, `gammals`, and binomial/Poisson/Gamma P-IRLS. Three-model ANOVA covers Gaussian, binomial, Poisson, Gamma, `gaulss`, and `gammals`. |
