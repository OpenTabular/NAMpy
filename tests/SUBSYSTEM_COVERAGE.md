...
| General-family fixed-smoothing / postprocess | `nampy/gam/fit/solvers/general_family/fixed_smoothing.py`, `.../newton.py` | `tests/families/test_gam_general_family_owner_contracts.py`, `tests/optimization/test_mgcv_fixed_inner_fit_parity.py`, `tests/families/test_general_family_mgcv_parity.py`, `tests/parity/test_mgcv_under_tested_supported_combinations.py` | Owner precedence plus mgcv `gam.fit5` parity; single structured `re`, `fs`, and linked-`sz` cases are covered, including fixed and optimized `fs` behavior and an `fs` block in linear predictor two. |
...
| 6. Fitting and smoothing selection | The combination matrix covers Gaussian, binomial, Poisson, Gamma, negative-binomial, `gaulss`, and `gammals` fits across GCV/ML/REML and supported optimizers; weights, offsets, select penalties, boundary SPs, zero-weight rejection, and near-separated binomial behavior are exercised. |
| 7. Prediction, inference, and diagnostics | Term filters plus unconditional SE; unconditional term inference for random effects, tensors, linked terms, FS, SZ, factor-by, and `ti`; structured newdata; explicit unseen-level and NA rejection; and diagnostics/snapshot round trips. Gaussian-inverse REML directly covers post-fit likelihood criteria, covariance modes, residuals, summary/ANOVA, and k-check. Persistence covers random effects, `te`, `ti`, linked terms, FS, SZ, factor-by, `gaulss`, `gammals`, and binomial/Poisson/Gamma P-IRLS. Three-model ANOVA covers Gaussian, binomial, Poisson, Gamma, `gaulss`, and `gammals`. |
