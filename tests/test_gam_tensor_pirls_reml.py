...
@pytest.mark.parametrize(
    ...
    [
        (
            "poisson",
            lambda: _make_poisson_data(seed=53, n=220),
            'y ~ te(x0, x1, bs=["cr", "cr"], k=[6, 6])',
        ),
        (
            "poisson",
            lambda: _make_poisson_data(seed=57, n=220),
            'y ~ ti(x0, x1, bs=["cr", "cr"], k=[6, 6])',
        ),
        (
            "binomial",
            lambda: _make_binomial_data(seed=67, n=220),
            'y ~ te(x0, x1, bs=["cr", "cr"], k=[6, 6])',
        ),
        (
            "gamma",
            lambda: _make_gamma_data(seed=97, n=220),
            'y ~ te(x0, x1, bs=["cr", "cr"], k=[6, 6])',
        ),
        (
            {"name": "negbin", "theta": 1.0},
            lambda: _make_negbin_data(seed=61, n=240, theta=1.0),
            'y ~ te(x0, x1, bs=["cr", "cr"], k=[6, 6])',
        ),
        (
            {"name": "negbin", "theta": 1.0},
            lambda: _make_negbin_data(seed=63, n=240, theta=1.0),
            'y ~ ti(x0, x1, bs=["cr", "cr"], k=[6, 6])',
        ),
    ],
)
def test_known_scale_tensor_fixed_sp_reml_derivatives_match_mgcv(
    family, data_factory, formula
):
    data = data_factory()
    ...
    grad = np.asarray(
        criterion_gradient(gam, gam.y_, log_sp, method="reml"), dtype=np.float64
    )
    np.testing.assert_allclose(
        grad,
    ...
    hess = np.asarray(
        criterion_hessian(gam, gam.y_, log_sp, method="reml"), dtype=np.float64
    )
    np.testing.assert_allclose(
        hess,
...
@pytest.mark.parametrize(
    ...
    [
        (
            "poisson",
            _make_poisson_data(seed=71, n=220),
            'y ~ t2(x0, x1, bs=["cr", "cr"], k=[6, 6])',
        ),
        (
            "binomial",
            _make_binomial_data(seed=73, n=220),
            'y ~ t2(x0, x1, bs=["cr", "cr"], k=[6, 6])',
        ),
        (
            "gamma",
            _make_gamma_data(seed=101, n=220),
            'y ~ t2(x0, x1, bs=["cr", "cr"], k=[6, 6])',
        ),
        (
            {"name": "negbin", "theta": 1.0},
            _make_negbin_data(seed=79, n=240, theta=1.0),
            'y ~ t2(x0, x1, bs=["cr", "cr"], k=[6, 6])',
        ),
    ],
)
