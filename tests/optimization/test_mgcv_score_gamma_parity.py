...
class TestMgcvScoreGammaParity:
    ...
    def test_gaussian_exact_reml_fixed_sp_matches_mgcv(self):
        ...
        gam_lo, y_lo, log_sp_lo = _fit_fixed_sp_model(
            data,
            formula,
            "gaussian",
            sp,
        ...
        gam_hi, y_hi, log_sp_hi = _fit_fixed_sp_model(
            data,
            formula,
            "gaussian",
            sp,
        ...
        assert gam_lo._resolve_ml_reml_scoring_backend("reml") == "gaussian_exact"
        assert gam_hi._resolve_ml_reml_scoring_backend("reml") == "gaussian_exact"

        actual_lo = float(criterion_value(gam_lo, y_lo, log_sp_lo, method="reml"))
        actual_hi = float(criterion_value(gam_hi, y_hi, log_sp_hi, method="reml"))

        assert np.isfinite(actual_lo)
        assert np.isfinite(actual_hi)
        assert abs(actual_hi - actual_lo) > 1e-6
