...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            np.array([0.85], dtype=np.float64),
        ),
        (
...
def test_poisson_tensor_vector_fx_fixed_sp_inner_state_matches_mgcv(formula, sp):
    """Verify that poisson tensor vector fx fixed sp inner state matches mgcv."""
    data = _make_poisson_data(seed=902, n=180)
    ...
    _assert_allclose(sol.eta, expected["linear_predictors"], atol=1e-8)
    _assert_allclose(sol.mu, expected["fitted_values"], atol=1e-8)
    deviance_atol = 5e-7 if "ti(" in formula else 1e-8
    _assert_allclose(sol.deviance, expected["deviance"], atol=deviance_atol)
    weight_atol = 1e-5 if "ti(" in formula else 5e-8
    _assert_allclose(sol.fisher_weights, expected["weights"], atol=weight_atol)
    _assert_allclose(sol.working_weights, expected["working_weights"], atol=weight_atol)
    if "ti(" in formula:
        stable = np.asarray(expected["weights"], dtype=np.float64) > 1e-3
        _assert_allclose(
            np.asarray(sol.working_response, dtype=np.float64)[stable],
            np.asarray(expected["working_response"], dtype=np.float64)[stable],
            atol=5e-5,
        )
    else:
        _assert_allclose(sol.working_response, expected["working_response"], atol=2e-7)
    _assert_allclose(kernel.dVkk, expected["dVkk"], atol=5e-8)
    db_drho_atol = 1e-6 if "ti(" in formula else 5e-8
    _assert_allclose(
        _expand_dbeta_to_original_space(kernel), expected["db_drho"], atol=db_drho_atol
    )
...
def test_gaulss_two_cr_gam_fit5_fixed_sp_inner_state_matches_mgcv_ml():
    """Verify that gaulss two-cr gam_fit5 fixed-smoothing ML state matches mgcv."""
    data = _gaulss_two_smooth_data(seed=33, n=140)
    formula = ['y ~ s(x, bs="cr", k=6) + s(z, bs="cr", k=6)', "~ 1"]
    snapshot = _run_mgcv_snapshot(data, formula, "gaulss", "ML")
    ...
    expected = _run_mgcv_fit5_fixed_sp(
        data,
        formula,
        "gaulss",
        sp,
    ...
    _assert_allclose(actual_coef_full, expected["coefficients_full"], atol=atol)
    _assert_allclose(actual_eta, expected["linear_predictors"], atol=atol)
    _assert_allclose(actual_fitted, expected["fitted_values"], atol=atol)
    _assert_allclose(-2.0 * float(fit["l"]), expected["deviance"], atol=atol)
    _assert_allclose(actual_db_drho_full, expected["db_drho_full"], atol=atol)
    assert float(fit["ldetHp"]) == pytest.approx(float(expected["ldetHp"]), abs=atol)
    assert (
        0.5
        * float(
            np.asarray(fit["coef_fit_space"], dtype=np.float64)
            @ (
                np.asarray(fit["St_full"], dtype=np.float64)
                @ np.asarray(fit["coef_fit_space"], dtype=np.float64)
            )
        )
    ) == pytest.approx(float(expected["penalty_quadratic"]), abs=atol)
    assert float(fit["score"]) == pytest.approx(
        float(snapshot["fit"]["criterion_value"]),
        abs=atol,
    )
    _assert_allclose(fit["score1"], snapshot["fit"]["outer_grad"], atol=atol)
    _assert_allclose(fit["score2"], snapshot["fit"]["outer_hess"], atol=hess_atol)

    actual_score = float(criterion_value(gam, y, np.log(sp), method="ml"))
    actual_grad = np.asarray(criterion_gradient(gam, y, np.log(sp), method="ml"))
    actual_hess = np.asarray(criterion_hessian(gam, y, np.log(sp), method="ml"))

    assert actual_score == pytest.approx(
        float(snapshot["fit"]["criterion_value"]),
        abs=atol,
    )
    _assert_allclose(actual_grad, snapshot["fit"]["outer_grad"], atol=atol)
    _assert_allclose(actual_hess, snapshot["fit"]["outer_hess"], atol=hess_atol)
