...
def test_capabilities_and_protocols():
    reg = _fixed_regressor()
    clf = GAMClassifier()

    assert isinstance(reg, SupportsCapabilities)
    assert isinstance(reg, PersistableModel)

    assert reg.capabilities() == Capabilities(
