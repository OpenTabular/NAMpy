...
# ─────────────────────────────────────────────────────────────────────────────
# 12. Parity tests for edge cases not covered elsewhere
# ─────────────────────────────────────────────────────────────────────────────

class TestParityFixedSmoothingParams:
    """Verify that fixed smoothing parameters are respected exactly."""

    def test_fixed_sp_does_not_change_during_fit(self):
        data = _make_gaussian_data(n=80)
        sp_fixed = [42.0]
        gam = GAM(
            formula='y ~ s(x0, bs="cr", k=8)',
            optimize_smoothing=False,
            smoothing_params=sp_fixed,
        )
        gam.fit(data=data)
        np.testing.assert_allclose(gam.smoothing_params, sp_fixed, rtol=1e-10)

    def test_fixed_sp_changes_fit_relative_to_optimized(self):
        data = _make_gaussian_data(n=100)
        gam_fixed = GAM(
            formula='y ~ s(x0, bs="cr", k=8)',
            optimize_smoothing=False,
            smoothing_params=[1000.0],  # heavy penalization → very smooth
        )
        gam_optim = GAM(
            formula='y ~ s(x0, bs="cr", k=8)',
            optimize_smoothing=True,
        ...
        gam_optim.fit(data=data)
        # Predictions should differ
        pred_fixed = gam_fixed.predict(data)
        pred_optim = gam_optim.predict(data)
        assert not np.allclose(pred_fixed, pred_optim, atol=1e-3)


class TestParityByVariableSmooth:
    """Verify numeric by-variable smooth behavior."""

    def test_numeric_by_variable_changes_predictions(self):
        rng = np.random.default_rng(42)
        n = 80
        x0 = rng.uniform(0, 1, n)
        z = rng.uniform(-1, 1, n)  # numeric by
        y = np.sin(2 * np.pi * x0) * z + rng.normal(scale=0.1, size=n)
        data = pd.DataFrame({"y": y, "x0": x0, "z": z})

        gam_with_by = GAM(
            formula='y ~ s(x0, bs="cr", k=6, by=z)',
            optimize_smoothing=False,
            smoothing_params=[1.0],
        )
        gam_without_by = GAM(
        ...
        gam_with_by.fit(data=data)
        gam_without_by.fit(data=data)
        pred_by = gam_with_by.predict(data)
        pred_no_by = gam_without_by.predict(data)
        assert pred_by.shape == (n,)
        assert not np.allclose(pred_by, pred_no_by, atol=1e-3)
...
class TestParityParitySnapshotConsistency:
    """Snapshot invariants: recomputed criterion matches stored value."""

    def test_snapshot_criterion_is_consistent(self):
        data = _make_gaussian_data(n=80)
        gam = GAM(
            formula='y ~ s(x0, bs="cr", k=8)',
            optimize_smoothing=True,
        ...
        # The parity block contains a criterion_view sub-dict
        criterion_view = snap["parity"]["criterion_view"]
        np.testing.assert_allclose(
            criterion_view["stored_criterion_value"],
            criterion_view["recomputed_criterion_value"],
            rtol=1e-5,
            err_msg="Stored criterion value does not match recomputed value in snapshot",
        )

    def test_snapshot_predictions_shape(self):
        data = _make_gaussian_data(n=80)
        gam = GAM(
            formula='y ~ s(x0, bs="cr", k=6)',
            optimize_smoothing=True,
        ...
        gam.fit(data=data)
        snap = gam.parity_snapshot(X=data, include_covariances=False)
        assert np.asarray(snap["predictions"]["response"]).shape == (80,)
        assert np.asarray(snap["predictions"]["link"]).shape == (80,)

    def test_snapshot_coef_count_matches_design(self):
        data = _make_gaussian_data(n=80)
        gam = GAM(
        ...
        from nampy.gam.parity.snapshots import _get_core
        core = _get_core(gam)
        n_coef = core.design_.n_coef
        # coef_full in the snapshot includes n_coef smooth coefficients + 1 for intercept
        assert len(snap["fit"]["coef_full"]) == n_coef + 1


class TestParityMLvsREML:
    """ML and REML should give different smoothing params but both stable fits."""

    def test_ml_and_reml_differ_in_smoothing(self):
        data = _make_gaussian_data(n=120)
        formula = 'y ~ s(x0, bs="cr", k=8) + s(x1, bs="cr", k=6)'
        gam_ml = GAM(formula=formula, optimize_smoothing=True, smoothing_method="ML")
        gam_reml = GAM(formula=formula, optimize_smoothing=True, smoothing_method="REML")
        gam_ml.fit(data=data)
        gam_reml.fit(data=data)
        # Smoothing params may differ between ML and REML
        sp_ml = np.array(gam_ml.smoothing_params)
        sp_reml = np.array(gam_reml.smoothing_params)
        assert sp_ml.shape == sp_reml.shape
        # Both produce finite predictions
        pred_ml = gam_ml.predict(data)
        pred_reml = gam_reml.predict(data)
        assert np.all(np.isfinite(pred_ml))
        assert np.all(np.isfinite(pred_reml))


class TestParityPSplineBasis:
    """P-spline basis produces finite, correctly-shaped results."""

    def test_ps_basis_predictions_finite(self):
        data = _make_gaussian_data(n=80)
        gam = GAM(
            formula='y ~ s(x0, bs="ps", k=8)',
            optimize_smoothing=True,
            smoothing_method="REML",
        )
        gam.fit(data=data)
    ...
    def test_ps_basis_edf_in_range(self):
        data = _make_gaussian_data(n=100)
        gam = GAM(
            formula='y ~ s(x0, bs="ps", k=10)',
            optimize_smoothing=True,
            smoothing_method="REML",
        )
        gam.fit(data=data)
        assert 1.0 <= gam.edf_ <= 11.0  # edf includes intercept


class TestParityThinPlateSpline:
    """Thin-plate spline (tp) produces correct shapes."""

    def test_tp_basis_predictions_finite(self):
        data = _make_gaussian_data(n=80)
        gam = GAM(
...
class TestParityNewDataPrediction:
    """Predictions on new data must use the same basis_transform as training."""

    def test_predict_on_new_data_same_as_training(self):
        data = _make_gaussian_data(n=80)
        gam = GAM(
            formula='y ~ s(x0, bs="cr", k=8)',
            optimize_smoothing=True,
        ...
        gam.fit(data=data)
        pred_train = gam.predict(data)
        # Predict on the same data again (should match exactly)
        pred_new = gam.predict(data)
        np.testing.assert_allclose(pred_train, pred_new, rtol=0, atol=0)

    def test_predict_on_subset(self):
        data = _make_gaussian_data(n=80)
        gam = GAM(
            formula='y ~ s(x0, bs="cr", k=8)',
            optimize_smoothing=True,
        ...
        gam.fit(data=data)
        pred_full = gam.predict(data)
        pred_sub = gam.predict(data.iloc[:20])
        np.testing.assert_allclose(pred_full[:20], pred_sub, atol=1e-12)
...
class TestParityTermContributions:
    ...
    def test_term_contributions_plus_intercept_equal_prediction(self):
        data = _make_gaussian_data(n=80)
        gam = GAM(
            formula='y ~ s(x0, bs="cr", k=8) + s(x1, bs="cr", k=6)',
            optimize_smoothing=True,
        ...
        result = gam.predict_feature_vals(data)
        pred = gam.predict(data)
        # Sum term contributions + intercept
        term_keys = [k for k in result.keys() if k not in ["output", "intercept"]]
        contribs = np.column_stack([np.array(result[k]) for k in term_keys])
        total = contribs.sum(axis=1) + np.array(result["intercept"])
        np.testing.assert_allclose(total, pred, atol=1e-10)


class TestParityGCVCriterion:
    """GCV optimization produces finite, sensible smoothing parameters."""

    def test_gcv_fits_without_error(self):
        data = _make_gaussian_data(n=100)
        gam = GAM(
            formula='y ~ s(x0, bs="cr", k=8)',
            optimize_smoothing=True,
        ...
        assert np.all(np.array(gam.smoothing_params) > 0)

    def test_gcv_predictions_finite(self):
        data = _make_gaussian_data(n=100)
        gam = GAM(
            formula='y ~ s(x0, bs="cr", k=8)',
            optimize_smoothing=True,
...
class TestParityLinkedBasis:
    """Linked basis (shared smoothing parameter) produces consistent smoothing."""

    def test_linked_basis_single_sp(self):
        data = _make_gaussian_data(n=100)
        gam = GAM(
            formula='y ~ s(x0, bs="cr", k=6, id="shared") + s(x1, bs="cr", k=6, id="shared")',
            optimize_smoothing=True,
            smoothing_method="REML",
        )
        gam.fit(data=data)
        # With shared id, there should be only 1 smoothing parameter
        assert len(gam.smoothing_params) == 1
        assert np.isfinite(gam.smoothing_params[0])

    def test_linked_basis_predictions_finite(self):
        data = _make_gaussian_data(n=100)
        gam = GAM(
            formula='y ~ s(x0, bs="cr", k=6, id="lnk") + s(x1, bs="cr", k=6, id="lnk")',
            optimize_smoothing=True,
            smoothing_method="REML",
        )
        gam.fit(data=data)
...
class TestParityCubicCyclicSpline:
    """Cyclic cubic spline (cc) produces sensible results."""

    def test_cc_basis_predictions_finite(self):
        rng = np.random.default_rng(42)
        n = 80
        # Use periodic data for cc
...
class TestParityMultipleLinearPredictors:
    """For LSS-style multi-predictor models, each predictor is fitted correctly."""

    def test_two_smooth_cr_and_ps_together(self):
        data = _make_gaussian_data(n=100)
        gam = GAM(
            formula='y ~ s(x0, bs="cr", k=8) + s(x1, bs="ps", k=6)',
            optimize_smoothing=True,
        ...
        assert pred.shape == (100,)
        assert np.all(np.isfinite(pred))
        # Two separate smoothing parameters
