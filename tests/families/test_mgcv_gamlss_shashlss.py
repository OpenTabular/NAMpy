...
def test_gam_fit5_shashlss_convergence():
    ...
    z_norm = rng.standard_normal(n)
    y = mu_true + sig * delta * np.sinh(
        (1.0 / delta) * np.arcsinh(z_norm) + eps_true / delta
    )
...
def test_gam_public_api_shashlss_formula_list_fit():
    """Public GAM API fits a four-predictor shashlss model via formula list."""
    ...
    eps_true = 0.2
    phi_true = 0.1
    sigma = np.exp(tau_true)
    delta = np.exp(phi_true)
    z = rng.standard_normal(n)
    y = mu_true + (delta * sigma) * np.sinh(
        (1.0 / delta) * np.arcsinh(z) + eps_true / delta
    )
    ...
    assert_allclose(coef_full[0], 1.2, atol=0.4)
    assert_allclose(coef_full[1], 0.75, atol=0.35)
...
def test_shashlss_l3_fd():
    ...
    dl2_dmu = (
        _shash_l2_raw(y, mu0 + h, tau0, eps0, phi0, phi_pen, b)
        - _shash_l2_raw(y, mu0 - h, tau0, eps0, phi0, phi_pen, b)
    ) / (2.0 * h)
    dl2_dtau = (
        _shash_l2_raw(y, mu0, tau0 + h, eps0, phi0, phi_pen, b)
        - _shash_l2_raw(y, mu0, tau0 - h, eps0, phi0, phi_pen, b)
    ) / (2.0 * h)
    dl2_deps = (
        _shash_l2_raw(y, mu0, tau0, eps0 + h, phi0, phi_pen, b)
        - _shash_l2_raw(y, mu0, tau0, eps0 - h, phi0, phi_pen, b)
    ) / (2.0 * h)
    dl2_dphi = (
        _shash_l2_raw(y, mu0, tau0, eps0, phi0 + h, phi_pen, b)
        - _shash_l2_raw(y, mu0, tau0, eps0, phi0 - h, phi_pen, b)
    ) / (2.0 * h)
    ...
    Dmp = Dmt + De / (sig * sSp1) - delta * asinhZ * Dme
    Dtp = zsd * Dmp
    _Dpp = Dtp - delta * asinhZ * Dep + delta * (z / sSp1 - asinhZ) * De - 2.0 * phi_pen
...
def test_shashlss_l4_fd():
    ...
    dl3_dmu = (
        _shash_l3_raw(y, mu0 + h, tau0, eps0, phi0, phi_pen, b)
        - _shash_l3_raw(y, mu0 - h, tau0, eps0, phi0, phi_pen, b)
    ) / (2.0 * h)
    dl3_dtau = (
        _shash_l3_raw(y, mu0, tau0 + h, eps0, phi0, phi_pen, b)
        - _shash_l3_raw(y, mu0, tau0 - h, eps0, phi0, phi_pen, b)
    ) / (2.0 * h)
    dl3_deps = (
        _shash_l3_raw(y, mu0, tau0, eps0 + h, phi0, phi_pen, b)
        - _shash_l3_raw(y, mu0, tau0, eps0 - h, phi0, phi_pen, b)
    ) / (2.0 * h)
    dl3_dphi = (
        _shash_l3_raw(y, mu0, tau0, eps0, phi0 + h, phi_pen, b)
        - _shash_l3_raw(y, mu0, tau0, eps0, phi0 - h, phi_pen, b)
    ) / (2.0 * h)
    ...
    for name, _col, fd_arr, l3_col in checks:
        if "_sym" not in name:
            continue
    ...
    CC_a = np.cosh(dTasMe_a)
    SS_a = np.sinh(dTasMe_a)
    _sSp1_a = fam._sqrtX2pm(z_a, 1.0)
    _asinhZ_a = np.arcsinh(z_a)
    zsd_a = y - mu0
    ...
    _app21_a = 1.0 / np.exp(3.0 * tau0)
    aqq03_a = np.exp(-2.0 * tau0 - 2.0 * phi0)
    _aqq05_a = aqq03_a * abb3_a + 1.0
    aqq27_a = 1.0 / aff13_a
    _arr07_a = 1.0 / np.sqrt(np.exp(aff08_a) * zsd_a**2 + 1.0) ** 3
    _arr12_a = 1.0 / (np.exp(aff08_a) * zsd_a**2 + 1.0)
    _ass16_a = aii11_a - zsd_a / (np.exp(tau0) * agg15_a)
    ass23_a = 1.0 / CC_a
    ass28_a = 1.0 / aff13_a
    att19_a = zsd_a**4
    _avv19_a = aii11_a - abb4_a * zsd_a * abb6_a
    ayy14_a = -abb4_a * zsd_a * abb6_a
    ayy16_a = aii11_a + ayy14_a
    _ayy17_a = aii11_a + ayy14_a - aff14_a * ajj15_a * aii17_a
    _ayy24_a = ayy16_a**2
    azz19_a = zsd_a**5
    bdd07_a = np.sqrt(np.exp(aff08_a) * zsd_a**2 + 1.0)
    bdd08_a = 1.0 / bdd07_a**3
    bdd14_a = 1.0 / bdd07_a
    _bdd15_a = aii11_a - abb4_a * zsd_a * bdd14_a
    _bgg4_a = (dTasMe_a + eps0) - zsd_a / (
        np.exp(tau0) * np.sqrt(np.exp(2.0 * abb5_a) * zsd_a**2 + 1.0)
    )
    ...
    bkk33_a = 1.0 / CC_a**3
    bkk34_a = SS_a**3
    _bll16_a = np.exp(aff08_a - 2.0 * tau0)
    _bmm34_a = bkk33_a
    _bmm35_a = bkk34_a
    _bss21_a = 2.0 * aff14_a * abb3_a * aff15_a - 3.0 * bjj26_a * att19_a * app16_a
    bss23_a = -abb4_a * zsd_a * abb6_a
    bss25_a = aii11_a + bss23_a
    _bss26_a = aii11_a + bss23_a - aff14_a * ajj15_a * aff15_a
    _bss29_a = bss25_a**2
    _bss33_a = (
        -4.0 * aff14_a * zsd_a * aff15_a
        + 18.0 * bjj26_a * ajj15_a * app16_a
        - 15.0 * np.exp(-tau0 + 6.0 * abb5_a) * zsd_a**5 / agg15_a**7
    )
    _btt24_a = zsd_a**6
    _byy24_a = 2.0 * aff14_a * ajj15_a * aff15_a - 3.0 * bjj26_a * azz19_a * app16_a
    _byy35_a = (
        -6.0 * aff14_a * abb3_a * aff15_a
        + 21.0 * bjj26_a * att19_a * app16_a
        - 15.0 * np.exp(-tau0 + 6.0 * abb5_a) * zsd_a**6 / agg15_a**7
    )
    _bzz7_a = CC_a**2
    _bzz9_a = SS_a**2
    cbb09_a = 1.0 / agg15_a**5
    _cbb18_a = 2.0 * aff14_a * abb3_a * aii17_a - 3.0 * app14_a * att19_a * cbb09_a
    _cbb24_a = aii11_a + ayy14_a - aff14_a * zsd_a**3 * aii17_a
    _cdd24_a = zsd_a**7
    cll08_a = 1.0 / bdd07_a**5
    cll16_a = aii11_a + bhh13_a
    _cll17_a = cll16_a**2
    _cll18_a = 2.0 * aff14_a * ajj15_a * bdd08_a - 3.0 * app14_a * azz19_a * cll08_a
    _cll24_a = aii11_a + bhh13_a - aff14_a * ajj15_a * bdd08_a
    cmm12_a = -3.0 * app14_a * azz19_a * cbb09_a
    _cmm16_a = 2.0 * aff14_a * ajj15_a * aii17_a + cmm12_a
    _cmm23_a = aii11_a + ayy14_a + aff14_a * ajj15_a * aii17_a + cmm12_a
    _cmm28_a = (
        -4.0 * aff14_a * ajj15_a * aii17_a
        + 18.0 * app14_a * azz19_a * cbb09_a
    ...
    _cpp08_a = (cpp06_a + aii11_a) ** 2
    _cpp12_a = aii11_a + cpp06_a - np.exp(-tau0 + aff08_a) * zsd_a**3 / bdd07_a**3
    ...
    _cqq21_a = (
        bhh14_a
        + bhh13_a
        + aff14_a * ajj15_a * bdd08_a
        - 3.0 * app14_a * azz19_a * cll08_a
    )
    _cqq25_a = bhh14_a + bhh13_a + cqq12_a
    _cqq28_a = 1.0 / aff13_a
    crr18_a = (
        aii11_a
        + aoo09_a
        + aff14_a * ajj15_a * aii17_a
        - 3.0 * app14_a * azz19_a * cbb09_a
    )
    crr19_a = bii11_a**4
    crr21_a = bii15_a**2
    crr25_a = (
        aii11_a
        + aoo09_a
        - 3.0 * aff14_a * ajj15_a * aii17_a
        + 15.0 * app14_a * azz19_a * cbb09_a
        - 15.0 * np.exp(-tau0 + 6.0 * abb5_a) * zsd_a**7 / agg15_a**7
    )
    crr28_a = bii11_a**2
    _ccc23_a = (
    ...
    _ccc24_a = ayy16_a**3
    _ccc28_a = (
        -4.0 * aff14_a * abb3_a * aii17_a
        + 18.0 * app14_a * att19_a * cbb09_a
        - 15.0 * np.exp(-tau0 + 6.0 * abb5_a) * zsd_a**6 / agg15_a**7
    )

    # Compute each of the 35 l4 components analytically (mirrors family code)
    j2_a = (
        -(6.0 * bjj14_a * app10_a * abb9_a**4) / abb8_a**4
        - (12.0 * bjj24_a * zsd_a * app16_a * abb9_a**3) / abb8_a**3
        + 8.0 * bjj14_a * app10_a * aqq27_a * aff17_a
        + 4.0 * app08_a * app10_a * aqq27_a * aff17_a
        - 15.0 * bjj08_a * abb3_a * bjj11_a * aqq27_a * aff17_a
        - 4.0 * bjj14_a * app10_a * aff17_a
        + 4.0 * app08_a * app10_a * aff17_a
        - 15.0 * bjj08_a * abb3_a * bjj11_a * aff17_a
        - 9.0 * bjj26_a * zsd_a * app16_a * abb8_a * abb9_a
        + 24.0 * bjj24_a * zsd_a * app16_a * abb8_a * abb9_a
        + 15.0 * bjj18_a * ajj15_a * bjj21_a * abb8_a * abb9_a
        + 9.0 * bjj26_a * zsd_a * app16_a * agg17_a * abb9_a
        + 12.0 * bjj24_a * zsd_a * app16_a * agg17_a * abb9_a
        - 15.0 * bjj18_a * ajj15_a * bjj21_a * agg17_a * abb9_a
        - 4.0 * bjj14_a * app10_a * aff13_a
        + 4.0 * app08_a * app10_a * aff13_a
        - 15.0 * bjj08_a * abb3_a * bjj11_a * aff13_a
        - 2.0 * bjj14_a * app10_a
        - 4.0 * app08_a * app10_a
        + 15.0 * bjj08_a * abb3_a * bjj11_a
        + (6.0 * np.exp((-4.0 * tau0) - 4.0 * phi0)) / app04_a**2
        - (48.0 * np.exp((-6.0 * tau0) - 6.0 * phi0) * abb3_a) / app04_a**3
        + (48.0 * np.exp((-8.0 * tau0) - 8.0 * phi0) * zsd_a**4) / app04_a**4
    )
    n3_a = (
        -(6.0 * SS_a**4) / CC_a**4
        + (8.0 * SS_a**2) / CC_a**2
    ...
    r3_a = (
        -(6.0 * crr19_a * abb9_a**4) / abb8_a**4
        + (12.0 * crr28_a * bii15_a * abb9_a**3) / abb8_a**3
        - 3.0 * crr21_a * ass28_a * aff17_a
        + 8.0 * crr19_a * ass28_a * aff17_a
        - 4.0 * bii11_a * crr18_a * ass28_a * aff17_a
        - 3.0 * crr21_a * aff17_a
        - 4.0 * crr19_a * aff17_a
        - 4.0 * bii11_a * crr18_a * aff17_a
        - 24.0 * crr28_a * bii15_a * abb8_a * abb9_a
        - crr25_a * abb8_a * abb9_a
        - 12.0 * crr28_a * bii15_a * ass23_a * abb9_a
        + crr25_a * ass23_a * abb9_a
        - 3.0 * crr21_a * aff13_a
        - 4.0 * crr19_a * aff13_a
        - 4.0 * bii11_a * crr18_a * aff13_a
        + 3.0 * crr21_a
        - 2.0 * crr19_a
        + 4.0 * bii11_a * crr18_a
        - (8.0 * abb1_a * abb3_a) / aff04_a
        + (56.0 * np.exp((-4.0 * tau0) - 4.0 * phi0) * zsd_a**4) / aff04_a**2
        - (96.0 * np.exp((-6.0 * tau0) - 6.0 * phi0) * zsd_a**6) / aff04_a**3
        + (48.0 * np.exp((-8.0 * tau0) - 8.0 * phi0) * zsd_a**8) / aff04_a**4
    )

    # Spot-check j2 (mmmm), n3 (eeee), r3 (pppp) against FD
    assert_allclose(
        j2_a, dl3_dmu[:, 0], rtol=tol_r, atol=1e-6, err_msg="j2(mmmm) analytic vs FD"
    )
    assert_allclose(
        n3_a, dl3_deps[:, 16], rtol=tol_r, atol=1e-6, err_msg="n3(eeee) analytic vs FD"
    )
    assert_allclose(
        r3_a, dl3_dphi[:, 19], rtol=tol_r, atol=1e-6, err_msg="r3(pppp) analytic vs FD"
    )
