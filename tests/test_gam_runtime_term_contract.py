...
def test_factory_supports_select_for_categorical_runtime_terms():
    ...
    specs = [
        TermSpec(
            kind="smooth",
            features=("area",),
            basis_options={"special": "s", "bs": "mrf", "k": 4, "select": True, "xt": {"penalty": np.eye(4)}},
            label="mrf_sel",
        ),
        TermSpec(
            kind="smooth",
        ...
        TermSpec(
            kind="smooth",
            features=("x0", "fac"),
            basis_options={"special": "s", "bs": "fs", "k": 7, "select": True, "xt": "cr"},
            label="fs_sel",
        ),
        TermSpec(
            kind="smooth",
            features=("x0", "fac"),
            basis_options={"special": "s", "bs": "sz", "k": 7, "select": True, "xt": "cr"},
            label="sz_sel",
        ),
    ]
...
def test_random_effect_term_rejects_linked_ids_with_mgcv_message():
    with pytest.raises(NotImplementedError, match=r"random effects don't work with ids\."):
...
def test_t2_invalid_term_sp_length_warns_and_is_ignored():
    ...
    spec = TermSpec(
        kind="smooth",
        features=("x0", "x1"),
        basis_options={"special": "t2", "bs": ["ps", "ps"], "k": [5, 5], "sp": [0.7, 1.3]},
...
def test_t2_select_adds_one_null_space_penalty():
    ...
    spec = TermSpec(
        kind="smooth",
        features=("x0", "x1"),
        basis_options={"special": "t2", "bs": ["tp", "cr"], "k": [6, 6], "select": True},
