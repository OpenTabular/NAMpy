...
class GAM:
    ...
    def _rk(self, x, z):
        return ((z - 0.5) ** 2 - 1 / 12) * ((x - 0.5) ** 2 - 1 / 12) / 4 - (
            (np.abs(x - z) - 0.5) ** 4 - (np.abs(x - z) - 0.5) ** 2 / 2 + 7 / 240
        ) / 24
    ...
    def _spl_X(self, x, xk):
        ...
        for i in range(2, q):
            X[:, i] = self._rk(x, xk[i - 2])  # and remaining columns to R(x, xk)
        return X
    ...
    def _spl_S(self, xk):
        ...
        for i in range(2, q):
            for j in range(2, q):
                S[i, j] = self._rk(xk[i - 2], xk[j - 2])  # fill in non-zero part
        return S

    def _setup(self, x, z, q=10):
        """Get X, S_1 and S_2 for a simple 2 term AM"""
        # choose knots
        xk = mstats.mquantiles(
            np.unique(x), prob=np.linspace(1 / (q - 1), 1 - 1 / (q - 1), q - 2)
        )
        zk = mstats.mquantiles(
            np.unique(z), prob=np.linspace(1 / (q - 1), 1 - 1 / (q - 1), q - 2)
        )

        # get penalty matrices
        S = [np.zeros((2 * q - 1, 2 * q - 1)), np.zeros((2 * q - 1, 2 * q - 1))]
        S[0][1:q, 1:q] = self._spl_S(xk)[1:, 1:]
        S[1][q : (2 * q - 1), q : (2 * q - 1)] = self._spl_S(zk)[1:, 1:]

        # get model matrix
        n = len(x)
        X = np.ones((n, 2 * q - 1))
        X[:, 1:q] = self._spl_X(x, xk)[:, 1:]  # 1st smooth
        X[:, q : (2 * q - 1)] = self._spl_X(z, zk)[:, 1:]  # 2nd smooth

        return {"X": X, "S": S}

    def fit(self, y, X, S, sp):
        ...
        while np.abs(norm - old_norm) > 1e-4 * norm:  # repeat until converged
            ...
            hat_matrix_diag = np.linalg.lstsq(X1.T @ X1, X1.T, rcond=None)[
                0
            ].diagonal()[:n]
    ...
    def apply_gcv(self, y, am_setup_output):
        ...
        for i in range(1, 31):  # loop over s.p. grid
            for j in range(1, 31):
                ...
                b = self.fit(
                    y, am_setup_output["X"], am_setup_output["S"], sp
                )  # fit using s.p.s
