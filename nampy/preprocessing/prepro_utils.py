...
class CustomBinner(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        # Store the bins calculated during fit for transform
        if isinstance(self.bins, int):
            _, self.bin_edges_ = pd.cut(X.squeeze(), bins=self.bins, retbins=True)
        else:
    ...
    def transform(self, X):
        ...
        binned_data = pd.cut(
            X.squeeze(),
            bins=np.sort(np.unique(self.bin_edges_)),
            labels=False,
            include_lowest=True,
        )
