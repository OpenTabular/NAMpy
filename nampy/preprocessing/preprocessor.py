...
class Preprocessor:
    ...
    def __init__(
        ...
        self.quantile_preprocessing = quantile_preprocessing.lower()
        if self.quantile_preprocessing not in ["feature", "global"]:
            raise ValueError(
                "Invalid quantile_preprocessing value. Supported values are 'feature' and 'global'."
            )

        self.quantile_output_distribution = quantile_output_distribution.lower()
        if self.quantile_output_distribution not in ["normal", "uniform"]:
            raise ValueError(
                "Invalid quantile_output_distribution value. Supported values are 'normal' and 'uniform'."
            )
    ...
    def set_params(self, **params):
        # Map parameter names to internal attribute names
        param_mapping = {"knots": "n_knots"}
    ...
    def fit(self, X, y=None):
        ...
        if numerical_features:
            for feature in numerical_features:
                ...
                if self.numerical_preprocessing in ["binning", "one_hot"]:
                    ...
                    if self.quantile_preprocessing == "feature":
        ...
        # Set up global quantile preprocessor if configured
        if (
            self.numerical_preprocessing == "quantile"
            and self.quantile_preprocessing == "global"
            and numerical_features
        ):
            # Extract numerical data and fit a global quantile transformer
            num_data = X[numerical_features].values
            # Handle missing values for fitting
            num_data_imputed = SimpleImputer(strategy="mean").fit_transform(num_data)
