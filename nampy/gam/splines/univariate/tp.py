...
def full_rank_shrinkage_penalty(S, shrink=1e-1, tol=1e-12):
    ...
    """
    return constant_null_space_shrinkage(S, shrink=shrink, tol=tol)
...
class ThinPlateBasisSetup:
    ...
    used_supplied_knots: bool = False
    used_subsampling: bool = False
    pure_knot: bool = False
...
def build_tprs_term_setup(
    ...
    *,
    ...
    return ThinPlateBasisSetup(
        ...
        used_supplied_knots=bool(setup_knots is not None),
        used_subsampling=bool(used_subsampling),
        pure_knot=bool(tprs["Xu"].shape[0] == tprs["k"]),
    )
