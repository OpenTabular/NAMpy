...
@register_smooth("sos")
class SphericalSplineTerm(BaseSmoothTerm):
    term_type = "smooth"
    basis_name = "sos"
    ...
    def fit(self, X, feature_names):
        ...
        self._setup = build_spherical_spline_setup(
            setup_values,
            k=self.k,
            m=self.m,
            knots=self.knots,
        ...
        base = (
            ...
            if shared_X is None
                ...
                predict_spherical_spline(values, self._setup), dtype=np.float64
            )
        )
        ...
        if self.pc is not None:
            constrained_basis, constrained_penalties, transform, _ = (
                self._apply_point_constraint(
                    ...
                    point_basis_fn=lambda points: predict_spherical_spline(
                        points, self._setup
                    )[0],
                    fixed=self.fixed,
                )
    ...
    def transform_new(self, X_new):
        ...
        basis = predict_spherical_spline(values, self._setup)
