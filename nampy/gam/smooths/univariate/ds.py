...
@register_smooth("ds")
class DuchonSplineTerm(BaseSmoothTerm):
    term_type = "smooth"
    basis_name = "ds"
    ...
    def fit(self, X, feature_names):
        ...
        if shared_X is not None:
            ...
            setup_values = values
        self._setup = build_duchon_spline_setup(
            setup_values,
            k=self.k,
            m=self.m,
            knots=self.knots,
        ...
        base = (
            ...
            if shared_X is None
                ...
                predict_duchon_spline(values, self._setup), dtype=np.float64
            )
        )
        ...
        if self.pc is not None:
            constrained_basis, constrained_penalties, transform, _ = (
                self._apply_point_constraint(
                    ...
                    point_basis_fn=lambda points: predict_duchon_spline(
                        points, self._setup
                    )[0],
                    fixed=self.fixed,
                )
    ...
    def transform_new(self, X_new):
        ...
        basis = predict_duchon_spline(values, self._setup)
