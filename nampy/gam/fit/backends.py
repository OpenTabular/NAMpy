...
from .capabilities import (
    model_fit_capabilities,
    validate_observation_transform_support,
)
...
def available_fit_backends(model):
    ...
    if capabilities.transformed_coefficients:
        backends.append("transformed_coefficients")
...
def resolve_fit_backend(model):
    ...
    if "transformed_coefficients" in backends:
...
def solve_fit(model, y, smoothing_params, backend=None, weights=None):
    validate_observation_transform_support(model)
    ...
    if backend in {"transformed_coefficients", "shape_constrained"}:
        return solve_transformed_coefficient_fit(
