...
from .general_newton_solver import (
...
class GeneralFamilySetupState:
    ...
    Mp: int
    score_type: str
    db_drho_sign_map: np.ndarray | None = None
...
def _canonicalize_eigenvector_signs(vectors: np.ndarray) -> np.ndarray:
    """Match mgcv-style deterministic eigenvector orientation per column."""
...
def _sl_single_penalty_block(
    ...
    *,
    ...
    # `mgcv::Sl.setup()` stores the raw `eigen(..., symmetric=TRUE)$vectors`
    # orientation for singleton non-diagonal blocks. General-family
    # `initial.spg()` uses those exact signs through family-specific starts.
...
def _sl_multi_penalty_block(
    ...
    *,
    ...
    order = np.argsort(values)[::-1]
    values = np.asarray(values[order], dtype=np.float64)
    # Keep the raw symmetric-eigenvector orientation by default. Upstream
    # `mgcv::Sl.setup()` stores `eigen(St, symmetric=TRUE)$vectors` directly
    # for ordinary multi-penalty blocks, and general-family `initial.spg()`
    # is sensitive to those exact signs. Tensor-ANOVA `t2` blocks need the
    # historical canonicalization used elsewhere in NAMpy to mirror mgcv's
    # stored raw basis orientation on the local test platform.
    vectors = np.asarray(vectors[:, order], dtype=np.float64)
    if str(sign_mode).lower() == "tensor_anova":
        vectors = _canonicalize_eigenvector_signs(vectors)
        n_tail = min(len(S_work), int(vectors.shape[1]))
        if n_tail > 0:
            tail = vectors[:, -n_tail:]
            tail_sums = np.sum(tail, axis=0)
            for j in range(n_tail):
                if float(tail_sums[j]) > 0.0:
                    vectors[:, vectors.shape[1] - n_tail + j] *= -1.0
    rank_use = _sl_rank_from_eigenvalues(values) if rank is None else int(rank)
    ...
    *,
    ...
    if isinstance(spec, dict):
        block = GeneralPenaltyBlock(
            ...
            sign_mode=str(spec.get("sign_mode", "raw")),
        )
    else:
        block = copy(spec)
        ...
        if getattr(block, "n_sp", None) is None:
            block.n_sp = int(n_sp)
        if getattr(block, "sign_mode", None) is None:
            block.sign_mode = "raw"
        ...
        if split_ok:
            ...
            _append_block(
                _sl_multi_penalty_block(
                    ...
                    repara=True,
                    penalty_indices=tuple(int(v) for v in term_penalty_idx),
                    sign_mode=str(getattr(term, "term_type", "")),
                )
            )
...
def sl_inirep(
    ...
    *,
    ...
    """Mirror ``mgcv::Sl.inirep`` for the implemented linear ``Sl`` blocks."""
    ...
    def _r_recycle_matrix_assign(rhs: np.ndarray, shape: tuple[int, int]) -> np.ndarray:
        ...
        if rhs_arr.shape == shape:
            return rhs_arr
        flat = (
            np.ravel(rhs_arr, order="F")
            if rhs_arr.ndim == 2
            else np.ravel(rhs_arr)
        )
    ...
    for block in Sl:
        ...
        if l:
            if is_matrix:
                target = X_arr[ind, :]
                if l == 1:
                    X_arr[ind, :] = _r_recycle_matrix_assign(D @ target, target.shape)
                elif l == 2:
                    X_arr[ind, :] = _r_recycle_matrix_assign(
                        D.T @ target, target.shape
                    )
                elif l == -1:
                    X_arr[ind, :] = _r_recycle_matrix_assign(
        ...
        if r:
            if is_matrix:
                target = X_arr[:, ind]
                if l == 1:
                    X_arr[:, ind] = _r_recycle_matrix_assign(target @ D, target.shape)
                elif l == 2:
                    X_arr[:, ind] = _r_recycle_matrix_assign(
                        target @ D.T, target.shape
                    )
                elif l == -1:
                    X_arr[:, ind] = _r_recycle_matrix_assign(
    ...
    for block in sl_blocks:
        ...
        for j, sp_idx in enumerate(smoothing_indices):
            derivs[sp_idx] += np.asarray(
                _sl_mult([block], eye_p, j + 1, full=True), dtype=np.float64
            )
...
def _general_family_t2_db_drho_sign_map(
    model,
    layout: _GeneralPredictorLayout,
) -> np.ndarray | None:
    # Current raw `gam.fit5` parity gap is limited to the two-linear-predictor
    # general-family `t2(..., bs=["tp", "cr/cs"])` path, where the stored
    # `db.drho` basis follows the alternative mgcv marginal sign convention
    # without affecting the fitted predictor basis itself.
    if len(layout.jj) != 2:
        return None

    from ...smooths.tensor.marginals import tensor_marginal_predict_matrix
    from ...smooths.tensor.t2 import TensorANOVASplineTerm
    from ...smooths.tensor.t2_basis import (
        build_tensor_anova_basis_and_penalties,
        materialize_tensor_anova_newdata,
    )

    X_train = np.asarray(getattr(model, "X_", None), dtype=np.float64)
    if X_train.ndim != 2 or X_train.shape[0] == 0:
        return None

    sign = np.ones(int(layout.X_full.shape[1]), dtype=np.float64)
    changed = False
    full_idx = np.asarray(layout.reduced_to_full_idx, dtype=int)

    for term in _term_blocks_seq(model):
        if str(getattr(term, "term_type", "")) != "tensor_anova":
            continue

        metadata = dict(getattr(term, "metadata", {}) or {})
        term_spec = metadata.get("term_spec", None)
        if not isinstance(term_spec, dict):
            continue
        basis_options = dict(term_spec.get("basis_options", {}) or {})
        if str(basis_options.get("special", "")).lower() != "t2":
            continue
        basis_list = [str(v).lower() for v in basis_options.get("bs", ())]
        if not any(name in {"cr", "cs"} for name in basis_list):
            continue

        basis_train = np.asarray(getattr(term, "basis_train", None), dtype=np.float64)
        if basis_train.ndim != 2 or basis_train.shape[1] == 0:
            continue

        feature_info = getattr(term, "feature_info", None)
        feature_names = (
            [] if feature_info is None else list(getattr(feature_info, "feature_names", ()))
        )
        if len(feature_names) == 0:
            continue

        runtime_term = TensorANOVASplineTerm(
            feature=term_spec["features"],
            k=basis_options.get("k", 10),
            basis=basis_options.get("bs", "cr"),
            m=basis_options.get("m", None),
            xt=basis_options.get("xt", None),
            label=term_spec.get("label", None),
            smoothing_id=term_spec.get("smoothing_id", None),
            by=term_spec.get("by_variable", None),
            sp=basis_options.get("sp", None),
            select=bool(basis_options.get("select", False)),
            full=bool(basis_options.get("full", False)),
            ord=basis_options.get("ord", None),
            fixed=bool(basis_options.get("fx", False)),
            knots=basis_options.get("knots", None),
        )
        runtime_term.fit(X_train, feature_names)

        alt_decomp = []
        for basis_name, dec in zip(runtime_term.basis, runtime_term._marginal_decompositions):
            dec_alt = {
                key: (
            ...
            if basis_key in {"cr", "cs"} and dec_alt["B_range"].shape[1] > 0:
                dec_alt["B_range"][:, 0] *= -1.0
                dec_alt["T_range"][:, 0] *= -1.0
            if basis_key == "cr" and dec_alt["B_null"].shape[1] >= 2:
                dec_alt["B_null"][:, -1] *= -1.0
                dec_alt["T_null"][:, -1] *= -1.0
            alt_decomp.append(dec_alt)

        t2_alt = build_tensor_anova_basis_and_penalties(
            alt_decomp,
            full=runtime_term.full,
            ord=runtime_term.ord,
            remove_constant_from_null_block=bool(runtime_term._by_state.is_constant),
        )
        marginal_fit = []
        for marginal_term, dec in zip(runtime_term._marginals, alt_decomp):
            B_raw = tensor_marginal_predict_matrix(marginal_term, X_train, centered=False)
            B_r = (
                B_raw @ dec["T_range"]
        ...
        alt_basis = materialize_tensor_anova_newdata(
            marginal_fit,
            allnull_specs=t2_alt["allnull_specs"],
            allnull_transform=t2_alt["allnull_transform"],
            penalized_specs=t2_alt["penalized_specs"],
        )
        alt_basis = runtime_term._apply_cached_by(np.asarray(alt_basis, dtype=np.float64))
        if alt_basis.shape != basis_train.shape:
            continue

        cols = np.asarray(full_idx[term.coef_slice], dtype=int)
        for j in range(basis_train.shape[1]):
            if np.allclose(
                basis_train[:, j],
                -alt_basis[:, j],
                rtol=0.0,
    ...
    return np.asarray(sign, dtype=np.float64) if changed else None
...
def build_general_family_setup_state(
    ...
    *,
    ...
    return GeneralFamilySetupState(
        ...
        db_drho_sign_map=_general_family_t2_db_drho_sign_map(model, layout),
    )
...
def solve_general_family_fit(model, y, smoothing_params, weights=None):
    ...
    cov_unconditional_space = (
        PREDICTION_PARAMETER_SPACE
        if prediction_parameterization_map(model) is not None
        else FIT_PARAMETER_SPACE
    )
