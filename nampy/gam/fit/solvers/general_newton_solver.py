    ...
    if Hsp is not None and db_drho is not None and db_drho.size > 0:
        ...
        for k in range(K):
            ...
            if db_drho_k.shape[0] == p and np.any(bdrop):
                ...
                db_drho_k = db_full
            if db_drho_sign_map is not None:
                sign_map = np.asarray(db_drho_sign_map, dtype=np.float64).ravel()
                if sign_map.shape[0] != db_drho_k.shape[0]:
                    raise ValueError(
                        "db_drho_sign_map length must match the postprocess coefficient space."
                    )
                db_drho_k = np.asarray(
                    db_drho_k * sign_map[:, np.newaxis],
                    dtype=np.float64,
                )
...
def _sl_initial_repara_local(Sl: Any, X: np.ndarray, **kwargs) -> np.ndarray:
    from .general_family_solver import sl_initial_repara

    return sl_initial_repara(Sl, X, **kwargs)


def _sl_inirep_local(Sl: Any, X: np.ndarray, **kwargs) -> np.ndarray:
    from .general_family_solver import sl_inirep
...
def _vb_corr_root(
    ...
    *,
    ...
    return np.asarray(
        _mgcv_vcorr(dR_inv, Vr_local, trans=False),
        dtype=np.float64,
    )
