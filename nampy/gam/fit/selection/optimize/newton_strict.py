...
def _optimize_outer_newton_strict(
    objective,
    x0,
    bounds,
    eval_at,
    *,
    ...
    old_score = score
    score_scale = _outer_newton_score_scale(
        score_type,
        score,
        old_score,
    ...
    def _curvature_diag(dvkk_val, hess_val, shape):
        dvkk_arr = np.asarray(dvkk_val, dtype=np.float64)
        if dvkk_arr.shape == shape and np.all(np.isfinite(dvkk_arr)):
            return dvkk_arr.copy()
        if dvkk_arr.ndim == 2 and dvkk_arr.shape[0] == dvkk_arr.shape[1]:
            diag = np.diag(dvkk_arr).astype(np.float64, copy=True)
            if diag.shape == shape and np.all(np.isfinite(diag)):
                return diag
    ...
    for nit in range(1, int(max_iter) + 1):
        ...
        # Python objective gradient is stored in criterion convention; the
        # steepest fallback must move in descent direction for this port.
    ...
    result.outer_qerror_thresh = float(qerror_thresh)
    result.edge_correction_requested = bool(edge_correct)
    result.edge_correction_applied = bool(edge_corrected)
