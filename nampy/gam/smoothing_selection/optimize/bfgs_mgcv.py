...
def _extract_dvkk_diag(model, x_eval, hess):
    ...
    if isinstance(kernel_state, dict):
        ...
        if dvkk is not None:
            ...
            if dvkk.shape == np.asarray(x_eval, dtype=np.float64).ravel().shape:
                return dvkk.copy()
    if hess is not None:
...
def _finite_difference_initial_inverse_hessian(
    ...
    *,
    ...
    B = 0.5 * (B + B.T)
    evals, evecs = scipy_eigh(B, check_finite=False, driver="evr")
    evals = np.abs(evals)
...
def _invert_inverse_hessian(B_inv):
    ...
    evals, evecs = scipy_eigh(B_inv, check_finite=False, driver="evr")
    if evals.size == 0:
        ...
        for i in range(1, int(max_step) + 1):
            ...
            if ms <= 0.0:
                ...
                if trial is None:
                    ...
                    uconv = uconv | (
                        np.abs(np.asarray(initial["dVkk"], dtype=np.float64))
                        > (score_scale * conv_tol * 0.1)
                    )
                    if np.all(uconv):
                        break
