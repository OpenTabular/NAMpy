...
from ...gam.linalg import symmetrize_matrix
...
def choose_gp_setup_locations(
    ...
    - If knots are supplied, use them directly after applying the same
      centering shift as the training covariates, unless there are more
    ...
    if knots is not None:
        ...
        if shift is not None:
            shift = np.asarray(shift, dtype=np.float64).reshape(1, d)
            K = K - shift
...
def gp_setup_from_data(
    ...
    *,
    ...
    penalty = symmetrize_matrix(penalty)
    ...
    return {
        ...
        "used_subsampling": bool(
            knots is None and n > int(max_knots) and xu.shape[0] > int(max_knots)
        ),
    }
...
def build_gp_term_setup(
    ...
    *,
    ...
    return GPBasisSetup(
        ...
        penalty=np.asarray(symmetrize_matrix(setup["penalty"]), dtype=np.float64),
