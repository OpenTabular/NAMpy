...
class EmbeddingLayer(nn.Module):
    ...
    def forward(self, num_features=None, cat_features=None):
        ...
        """
        # Determine batch_size for CLS token if needed
        batch_size = None
        if self.use_cls:
            if cat_features is not None and len(cat_features) > 0:
                batch_size = cat_features[0].size(0)
            elif num_features is not None and len(num_features) > 0:
        ...
        if self.use_cls:
            # Get batch_size from embeddings if not already determined
